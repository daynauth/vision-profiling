...
mem_size = int(os.environ.get('MEM_SIZE', 2))
memory_constrain = 1024*mem_size
        ...
        for bandwidth in self.bandwidth_list:
            ...
            for num_devices in range(1, self.num_devices_max + 1):
                ...
                if not (opt1.success or opt2.success or opt3.success or opt4.success):
                    #print(f"Failed to optimize {self.config} with bandwidth {bandwidth} and num_devices {num_devices}")
                    continue
            ...
            if len(across_devices) == 0:
                #print(f"Failed to optimize {self.config} with bandwidth {bandwidth}")
                return False
...
def driver(config, threshold):
    ...
    if success == False:
        print(f"Failed to optimize {config}")
        exit(1)
